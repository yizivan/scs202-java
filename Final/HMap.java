...
public class HMap<K, V>  implements MapInterface<K,V>
{
  ...
  protected LinkList<MapEntry<K,V>>[] mapLinkList; 
  protected BinarySearchTree<MapEntry<K,V>>[] BSTree;
  ...
  public void fixedSize(int size)
  {
    currCap = size;
    mapLinkList = new LinkList[currCap];
    for (int i =0; i < currCap; i++)
    {
      mapLinkList[i] = new LinkList<MapEntry<K,V>>();
    }
    BSTree = new BinarySearchTree[currCap];
    for (int i = 0; i < currCap; i++)
    {
      BSTree[i] = new BinarySearchTree<MapEntry<K,V>>();
    }
  }
  ...
  public V putLL(K k, V v)
  {
    ...
    MapEntry<K, V> entry = new MapEntry<K, V>(k, v);
    int location = Math.abs(k.hashCode()) % currCap;
    if (mapLinkList[location].search(entry) == null)
    {
      LLNode<MapEntry<K,V>> newNode = new LLNode<MapEntry<K,V>> (null, entry, null);
      mapLinkList[location].add(newNode);
      return v;
    }
    else 
    {
  ...
  public V putBST(K k, V v)
  {
    ...
    MapEntry<K,V> entry = new MapEntry<K,V>(k,v);
    int location = Math.abs(k.hashCode()) % currCap;
    if (BSTree[location].contains(entry) == null)
    {
      BSTree[location].add(entry);
      return v;
    }
    else 
    {
  ...
  public V getLL(K k)
  {
    ...
    if (mapLinkList[location].search(entry) != null)
    {
      return mapLinkList[location].search(entry).info.value;
    }
    else
    {
      return null;
  ...
  public V getBST(K k)
  {
    ...
    if (BSTree[location].contains(entry) !=null)
    {
      return BSTree[location].contains(entry).getInfo().value;
    }
    else
    {
      return null;
  ...
  public boolean removeBST(K k)
  {
    ...
    if (BSTree[location].contains(entry) !=null)
    {
      v = BSTree[location].contains(entry).getInfo().value;
    }
    else 
      return false;
    entry = new MapEntry<K,V>(k, v);
    return (BSTree[location].remove(entry));
  ...
  public double getAveChainLengthLL()
  {
    ...
    for (int i = 0; i < currCap; i++)
    {
      aveLength = aveLength + mapLinkList[i].getLength();
    }
    aveLength = aveLength / currCap;
    return aveLength;
  }
  ...
  public double getAveChainLengthBST()
  {
    ...
    for (int i = 0; i < currCap; i++)
    {
      aveLength = aveLength + BSTree[i].size();
    }
    aveLength = aveLength / currCap;
    return aveLength;
  }
  ...
  {
    double mean = getAveChainLengthLL();
    double squTotal = 0;
    for (int i = 0; i < currCap; i++)
    {
      squTotal = squTotal + ((mapLinkList[i].getLength() - mean) * (mapLinkList[i].getLength() - mean));
    }
    double variance = squTotal / (currCap - 1);
  ...
  {
    double mean = getAveChainLengthBST();
    double squTotal = 0;
    for (int i = 0; i < currCap; i++)
    {
      squTotal = squTotal + ((BSTree[i].size() - mean) * (BSTree[i].size() - mean));
    }
    double variance = squTotal / (currCap - 1);
    double sd = Math.sqrt(variance);
    return sd;
